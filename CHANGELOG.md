...
- Eight more agent backends (`bridge/providers.js`): the official **Grok Build CLI** (`grok -p`, xAI cloud — distinct from the local `grok-local` fork), **Hermes** (`hermes -z`, session id recovered via `--usage-file` for resume), **OpenClaw** (`openclaw agent --local -m`, `--session-id` resume), **Codex** (`codex exec --json`, `exec resume --json` for resume), **Gemini** (`gemini --output-format stream-json -p`), **OpenCode** (`opencode run --format json`, `--session` resume), and **MiniMax Code** (`mcode exec --output-format stream-json`, `--session` resume). The headless JSONL output protocols for Codex/Gemini/OpenCode/MiniMax Code get dedicated parsers in `bridge/output.js` (session, text deltas, progress, errors, terminal records) following ZCode's harness driver shapes — see [NOTICE.md](NOTICE.md) for credit. The existing `zcode` provider was verified to already wrap the `zcode-local` headless CLI (`zcode-local -p --mode … --resume …`). All eight are listed in `setup.js --provider` and the README backends table. `hermes` is marked experimental — a live smoke test needs a model loaded and none was available during testing (no model is ever loaded as a side effect). `openclaw` was verified live on Windows (`openclaw agent --local -m "Reply with exactly: OK"` → `OK`); the Mac CLI is currently blocked by a stale discord plugin on Ryan's lived-in config.
...
- Pure output parsers (`bridge/output.js`) for Claude's stream-json events and the Muse CLI's JSONL events, covered by `tests/providers_test.js` (provider arg construction, no silent backend switching, no hard-coded model IDs, SystemOne success/timeout/error fail-open).
...
- **Claude de-emphasized:** the bridge no longer silently falls back to Claude Code when the Muse CLI is missing — a missing backend executable now fails loudly with setup guidance listing the available backends. The `claude` provider remains for explicit selection and legacy `claudePath` configs. Docs (README, install, configuration, architecture) lead with Muse, grok-local, ZCode, the harness and LM Studio; `setup.js` accepts `--provider <id>` to pick the backend at install time.
...
[Unreleased]: https://github.com/Franzferdinan51/wow-muse/compare/v0.3.0...HEAD
[0.3.0]: https://github.com/Franzferdinan51/wow-muse/releases/tag/v0.3.0
