...
| `provider` | see below | Which agent backend runs each turn. `id` picks the provider (default `muse`; falls back to `claude` when the Muse CLI is missing), `path` overrides its executable, `model` is passed where the CLI supports it, `harnessProvider` selects the Custom-Code-Harness provider for `ch`, and `http` configures the HTTP providers (`baseUrl`, `apiKey`, `model`, `timeoutMs`). Full table in the README's [Backends](../README.md#backends) section. |
