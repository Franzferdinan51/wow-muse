...
- **Jobs:** one agent process per chat, up to `maxParallel` at once, queued per chat beyond that. The provider registry (`bridge/providers.js`) builds the command line per backend: Claude Code keeps its original `claude -p --output-format stream-json --verbose --permission-mode … --allowedTools … [--resume <id>]` with the prompt on stdin; Meta Muse gets the prompt as a temp file (`muse exec --json … --prompt-file`); grok-local, the Grok Build CLI, ZCode, `ch`, Hermes, OpenClaw, Codex, Gemini, OpenCode and MiniMax Code take it as an argument and print plain text or JSONL; LM Studio / generic HTTP get `POST /v1/chat/completions`. Tool-use events (or output lines) become progress lines (`edit player.gd`, `$ npm test`) and heartbeat files; the final result becomes the reply. Session ids are stored per chat id in `state.json` for providers that support resume, so `--resume` survives an addon data reset.
...
- **Provider selection:** per turn, in this order: (1) the SystemOne router's advisory tier → `systemone.tierMap` (see below), (2) the `provider` block in the config, (3) explicit user config. The default is `muse`; if the selected backend's executable isn't installed the run fails loudly with setup guidance — the bridge never silently switches backends. A routed or explicit choice is honored as-is. Output handling follows the provider's `outputMode`: `stream-json` (the legacy Claude Code event protocol), `jsonl-text` (Muse CLI, parsed best-effort in `bridge/output.js`), `codex-json` / `gemini-json` / `opencode-json` / `mcode-json` (headless harness JSONL protocols, parsed by the `create*Parser` factories in `bridge/output.js`, following ZCode's harness driver shapes), or plain `text` (grok-local, the Grok Build CLI, ZCode, `ch`, Hermes, OpenClaw), and `http` for the OpenAI-compatible endpoints (`runHttpJob` POSTs `/v1/chat/completions` with Node's built-in `http`/`https`, no dependencies). No model IDs are hard-coded anywhere: the model always comes from the router decision or the config.
